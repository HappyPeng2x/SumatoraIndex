...
Furigana markup is generated by tokenizing each sentence with MeCab (via the
fugashi binding) against the UniDic dictionary directory produced by
unidic-to-git.py (-u flag). This covers every kanji in the sentence, unlike
relying on Tatoeba's own B-line annotations, which are manually curated and
often incomplete — see Algorithms.md. The B-line annotations (`indices` in
the sentence JSON) are still used, unchanged, to resolve which JMdict entries
a sentence links to.

Requires: `fugashi` (pip install fugashi) — no system libmecab needed, the
wheel bundles the MeCab core.
...
# ---------------------------------------------------------------------------
# MeCab tokenization (full-sentence furigana coverage)
# ---------------------------------------------------------------------------

# UniDic feature index for "kana": the orthographic reading (katakana, no
# phonetic contraction), e.g. 東京 → トウキョウ rather than the phonetic
# f[9] "pron" → トーキョー. This is the field conventional furigana uses.
# See Algorithms.md and unidic-to-git.py's module docstring for how this was
# determined.
_KANA_COL = 20


def _reading_of(word):
    """Return the hiragana reading of a fugashi word, or None if unavailable.

    Unknown words (not in the dictionary — numbers, rare proper nouns, etc.)
    have a short feature tuple from the unk-format and carry no kana field;
    symbols and punctuation have kana == '*'. Both cases return None, leaving
    that span of text unmarked, same as the rest of the pipeline's graceful
    fallback for unresolvable readings.
    """
...
class MecabTokenizer:
    """Tokenizes Japanese text into (writing, reading) pairs via MeCab/UniDic.

    dicdir must be a MeCab dictionary directory as produced by
    unidic-to-git.py (sys.dic, matrix.bin, char.bin, unk.dic, dicrc).
    """
    ...
    def tokenize(self, text):
        """Return a token list in the same shape markup_sentence expects.

        Surfaces are emitted in order and reconstitute the input text
        exactly, so markup_sentence's left-to-right matching always
        succeeds — every kanji in the sentence gets a chance at furigana,
        not just the ones covered by Tatoeba's own B-line annotations.
        """
        tokens = []
...
def process(gitoeba_dir, jmdict_path, unidic_dir, output_dir):
    ...
    for sent_id, (text, indices) in sentences.items():
        ...
        if seq_to_token:
            ...
            marked_cache[sent_id] = markup_sentence(text, tokenizer.tokenize(text))
    resolver.close()
...
HELP = (
    ...
    '-i <gitoeba directory> -j <jmdict.db> -u <unidic dicdir> -o <output directory>'
)
...
def main(argv):
    ...
    for opt, arg in opts:
        if opt == '-h':
            ...
            output_dir = arg
    if not gitoeba_dir or not jmdict_path or not unidic_dir or not output_dir:
        print(HELP)
        sys.exit(2)
    process(gitoeba_dir, jmdict_path, unidic_dir, output_dir)
