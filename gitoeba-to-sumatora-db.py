...
def process(gitoeba_dir, unidic_dir, db_path):
    ...
    for lang in lang_dirs:
        ...
        for path in iter_json_files(lang_dir):
            ...
            entry_links = entry_cache.get(sent_id)
            if not entry_links:
                continue
            ...
            for ord_, (entry_id, (form_id, matched_text, sense_id)) in enumerate(entry_links.items()):
                conn.execute(
                    'INSERT OR IGNORE INTO EntryExample '
                    '(entry_id, example_id, ord, matched_text, sense_id) VALUES (?, ?, ?, ?, ?)',
                    (entry_id, example_id, ord_, matched_text, sense_id),
                )
                link_count += 1
            example_count += 1
            lang_examples += 1
        print(f'  {lang}: {lang_examples} examples', flush=True)
